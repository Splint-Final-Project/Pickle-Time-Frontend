...
export const useDateSelect = create<DateInterface>(set => ({
  ...
  finishDate: { month: 1, day: 1},
  weekend: DAY_OF_WEEK,
  startTime: { hour: 1, minute: 0, dayTime: "AM"},
  finishTime: { hour: 1, minute: 0, dayTime: "AM"},
  setStartDate: (newStartDate: { month: number, day: number}) => set(state => ({ ...state, startDate: newStartDate })),
  setFinishDate: (newFinishDate: { month: number, day: number}) => set(state => ({ ...state, finishDate: newFinishDate })),
  setWeekend: (newWeekend: DayOfWeekInterface[]) => set(state => ({ ...state, weekend: newWeekend })),
  setStartTime: (newStartTime: { hour: number, minute: number, dayTime: string}) => set(state => ({ ...state, startTime: newStartTime })),
  setFinishTime: (newFinishTime: { hour: number, minute: number, dayTime: string}) => set(state => ({ ...state, finishTime: newFinishTime })),
}))